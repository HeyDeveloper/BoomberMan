...
public class Fire extends Goods{
    ...
    public Fire(Bomb bomb, ArrayList<JLabel> fire_set, StartGame gameframe,
    		int fire_center_x, int fire_center_y, int single_offset) {
        super(gameframe);
        this.bomb_obj = bomb;
        this.gameframe = gameframe;
        this.fire_set = fire_set;
    ...
    public void calculateExplodeArea(int bomb_x, int bomb_y) {
        ...
        int fire_area = single_offset * ONE_STEP; //fire_area为火焰偏移量
    ...
    private void explode(int bomb_x, int bomb_y, int offsetX, int offsetY) {
        ...
        if (gameframe.dates[map_x][map_y] == NORMAL_ROUTE || gameframe.dates[map_x][map_y] == BOMB_IN_ROUTE) {
